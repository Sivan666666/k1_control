...
def signal_handler(sig, frame):
    """捕获 SIGINT 信号（Ctrl+C）并优雅地退出程序"""
    global app_running
    rospy.loginfo("捕获到 Ctrl+C 信号，正在退出程序...")
    app_running = False
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ================         Demo2        ================ #
        # ================================================================= #
        while app_running and not rospy.is_shutdown():
            rospy.loginfo("\n======== Demo: 双臂轨迹运动 ========")
            if mode == "grasp":
                # 定义左臂和右臂的轨迹
                left_motion = [
                    pose_to_se3(0.3, 0.25, 0.15, 174, 12, 88),
                    pose_to_se3(0.3, 0.27, 0.70, 81, 26, 70)
                ]

                right_motion = [
                    pose_to_se3(0.3, -0.25, 0.15, 174, -12, 88),
                    pose_to_se3(0.3, -0.27, 0.70, 81, -26, 110)
                ]
            elif mode == "fling":
                # 定义左臂和右臂的轨迹
                left_motion = [
                    pose_to_se3(0.3, 0.25, 0.15, 174, 12, 88),
                    pose_to_se3(0.55, 0.27, 0.63, 80, -26, 70),
                    pose_to_se3(0.35, 0.25, 0.2, 174, 12, 88)
                ]

                right_motion = [
                    pose_to_se3(0.3, -0.25, 0.15, 174, -12, 88),
                    pose_to_se3(0.55, -0.27, 0.63, 80, -26, 110),
                    pose_to_se3(0.35, -0.25, 0.2, 174, -12, 88)
                ]

            # 遍历轨迹点，依次移动
            for i, (left_pose, right_pose) in enumerate(zip(left_motion, right_motion)):
                if not app_running:  # 检查是否需要退出
                    break
