...
class DualGripperROSNode:
    def __init__(self):
        ...
        # ROS话题订阅和发布
        rospy.Subscriber('gripper1_target_position', Int32, self._target_position_callback1)
        rospy.Subscriber('gripper2_target_position', Int32, self._target_position_callback2)
        ...
        # 状态跟踪变量
        self.current_target1 = None
        self.current_target2 = None
    ...
    def _init_single_gripper(self, device, port, baudrate, force, speed, gripper_id):
        ...
        try:
            ...
            gripper = dh_modbus_gripper.dh_modbus_gripper()
            gripper.set_device(device)  # 注入设备实例
            # 使用独立的device实例
    ...
    def _target_position_callback1(self, msg):
        """处理目标位置订阅消息"""
        target = msg.data
        rospy.loginfo(f"收到目标位置指令: {target}")
        self.current_target1 = target
        
        # 
        try:
            self.grippers[0].SetTargetPosition(target)
            self.is_moving[0] = True
        except Exception as e:
                rospy.logerr(f"夹爪{0} 控制失败: {str(e)}")


    def _target_position_callback2(self, msg):
        """处理目标位置订阅消息"""
        target = msg.data
        rospy.loginfo(f"收到目标位置指令: {target}")
        self.current_target2 = target
        
        try:
            self.grippers[1].SetTargetPosition(target)
            self.is_moving[1] = True
        except Exception as e:
                rospy.logerr(f"夹爪{1} 控制失败: {str(e)}")
