...
class dh_modbus_gripper(object):
    ...
    def open(self, portname, baudrate):
        ...
        if self.m_device is None:
            self.m_device = dh_device()  # 创建新实例
        print("1")
        return self.m_device.connect_device(portname, baudrate)
    ...
    def WriteRegisterFunc(self,index, value) :
        ...
        while ( ret == False ):
            ...
            wdlen = self.m_device.device_write(send_temp)
    ...
    def Initialization(self) :
        self.WriteRegisterFunc(0x0100,0xA5)  #0xA5 calibration 1 no calibration
...
# dh = dh_modbus_gripper()
# #dh.open("/dev/ttyUSB0",115200)
# dh.Initialization()
# dh.SetTargetPosition(0)
