...
class K1DualArmController:
    def __init__(self, urdf_path=None):
        ...
        self.end_pose_pub = rospy.Publisher("control_left", PoseStamped,queue_size=10)
        ...
        self.velocity_limits = np.array([np.deg2rad(10)] * 18)
        # 关节加速度限制 (rad/s^2)
        self.acceleration_limits = np.array([np.deg2rad(10)] * 18)
    ...
    def dual_arm_command_j_send_callback(self, event):
        ...
        if left_arm_active:
            ...
            self.end_pose_pub.publish(left_end_control)
    ...
    def run_fling_with_toppra(self):
        ...
        # 创建8ms定时器（只在轨迹生成后启动）
        self.timer = rospy.Timer(rospy.Duration(0.02), self.dual_arm_command_j_send_callback)
    ...
    def generate_fling_trajectory(self, arm='left'):
        ...
        if self.traj_index == 1:
            if arm == 'left':
                motion = [
                    pose_to_se3(0.38, 0.19, -0.21, 174.3, 12, 88.3),
                    pose_to_se3(0.38, 0.19, 0, 174.3, 12, 88.3),
                ...
                motion = [
                    pose_to_se3(0.38, -0.19, -0.21, 174, -12, 92),
                    pose_to_se3(0.38, -0.19, 0, 174, -12, 92),
